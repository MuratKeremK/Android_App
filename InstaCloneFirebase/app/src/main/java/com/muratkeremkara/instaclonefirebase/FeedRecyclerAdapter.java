...
public class FeedRecyclerAdapter extends RecyclerView.Adapter<FeedRecyclerAdapter.PostHolder> {
    ...
    public void onBindViewHolder(@NonNull PostHolder holder, int position) {
        ...
        Picasso.get().load(userImageList.get(position)).into(holder.imageView);// nereye yükleyeceğini anlatıyor // picasso uygulamsını incele
    }

    @Override
    public int getItemCount() {
        return userEmailList.size();// kaç tane row olacak onu yazazağız
    }

    class PostHolder extends RecyclerView.ViewHolder {// oluşturduğumuz değişkenler tanımlanıyor layouttaki
